...
*Только для администраторов:*
 - /filter <ключевое слово> <соответствующее сообщение>: Добавить фильтр в этот чат. Бот будет отвечать на это сообщение каждый раз, как "ключевое слово" \
будет упомянуто. 
ПРИМЕЧАНИЕ: все фильтры не уязвимы к регистру букв.
Если вы отвечаете стикером с ключевым словом, то ответьте на стикер сообщением `/filter 'ключевое слово'`. 
Если вы хотите, чтобы ваше ключевое слово было предложением, используйте одинарные кавычки. Например: `/filter 'Привет всех' Как вы?`
 - /stop <ключево слово>: Убрать этот фильтр.""",
...
"Word Blacklists_help":"""Черные списки используются для остановки определенных триггеров в группе. Каждый раз, когда упоминается триггер, \
сообщение будет мгновенно удалено. Это хорошо сочетается с предупреждающими фильтрами.

ПРИМЕЧАНИЕ: черные списки не распространяются на администраторов групп.

 - /blacklist: Просмотр текущих триггеров в группе.

*Только для администраторов:*
 - /addblacklist <тригер>: добавить триггер в черный список. Каждый термин начинается с одного тригера, \
таким образом, использование разных терминов позволит вам добавить несколько триггеров.
 - /unblacklist <тригер>: Удалите триггер из черного списка. Используйте ту же логику нового термина, здесь вы можете удалить \
несколько триггеров одновременно.
 - /rmblacklist <тригер>: то же самое, что и выше.""",
...
"Rules_help": """ - /rules: Показать правила чата.

*Только для администраторов:*
 - /setrules <правила>: Настройка правил чата.
 - /clearrules: Удаление правил для чата.""",
    
"Warnings_help": """ - /warns <ник>: Показать свои предупреждения или варны пользователя.
 - /warnlist: Вывести список фильтров для варнов.

*Только для администраторов:*
 - /warn <ник>: Предупредите пользователя. После 3 предупреждений пользователь будет заблокирован в группе. Может также использоваться в качестве ответа.
 - /resetwarn <ник>: Сброс предупреждения пользователя. Может также использоваться в качестве ответа.
 - /addwarn <ключове слово> <попередження>: Установите фильтр предупреждений на ключевое слово. Если вам нужно ключевое слово \
было предложение, выделите его кофе, например: '/addwarn Я злюсь 'Это злой пользователь'`.
 - /nowarn <ключове слово>: Отключение фильтра предупреждений.
 - /warnlimit <число>: Установка лимита предупреждений.
 - /strongwarn <on/yes/off/no>: Если флажок установлен, превышение лимита предупреждений приведет к блокировке. Если этот параметр отключен, пользователь будет выгнан.""",
...
#Muting
    "You'll need to either give me a username to mute, or reply to someone to be muted.":
        "Вам нужно либо дать мне имя пользователя, либо ответить на сообщение, чтобы он был замьючен.",
...
"Welcomes/Goodbyes_help": """Сообщения приветствия и прощания могут быть персонализированы многими методами. 
Если вы хотите, чтобы сообщения были индивидуально сгенерированные, например приветственное сообщение по умолчанию, \
вы можете использовать следующие переменные:
 ...
 - `{{mention}}`: Упоминает пользователя с именем.
 - `{{id}}`: ID пользователя.
 - `{{count}}`: Счетчик пользователей в группе.
 - `{{chatname}}`: Имя группы.
Каждая переменная должна быть выделена `{{}}` для изменения.
Сообщение Приветствия и Прощания также поддерживают markdown, поэтому вы можете создавать любые элементы. \
Кнопки также поддерживаются, поэтому вы можете создать свои приветствия великолепными с лоромогой некоторых кнопок с надписью. \
Чтобы создать кнопку, которая ссылается на ваши правила, используйте это: `[Правила](buttonurl://t.me/{}?Start=group_id)`.\
Просто измените `group_id` ID вашей группы, который можно получить через /id. Обратите внимание, что ID группы обычно предшествует `-`, он нужен, \
поэтому, пожалуйста, не выдавайте его. \
Вы можете также установить изображения/гифки/видео/голосовые сообщения в качестве приветственного сообщения \
дайте ответ на нужный элемент и напишите /setwelcome.

*Только для администраторов:*
 - /welcome <on/off>: Включить / выключить приветственные сообщения.
 - /welcome: показывает данный статус приветственных сообщений.
 - /welcome noformat: показывает данные настройки приветствия, без форматирования-полезно при изменении вашего приветственного сообщения!
 - /goodbye -> тоже самое что и /welcome.
 - /setwelcome <текст>: Установка пользовательского приветственного сообщения.
 - /setgoodbye <текст>: Установка пользовательского прощального сообщения.
 - /resetwelcome: Вернуть приветственное сообщение по умолчанию.
 - /resetgoodbye: Вернуть прощальное сообщение по умолчанию.
 - /cleanwelcome <on/off>: Удаление предыдущего приветственного сообщения после отправки нового, чтобы избежать спама в группе.
 - /cleanservice <on/off/yes/no>: Удаляет все служебные сообщения; Эти надоедливые \"Х присоединились к группе\", которые вы видите, когда люди присоединяются к группе.
 - /welcomesecurity <off/soft/hard>: soft - запретить пользователю отправлять медиафайлы в течение 24 часов, hard-запретить пользователю отправлять сообщения, пока он не нажмет кнопку \"Я не бот\".""".format(dispatcher.bot.username),
...
"Reporting_help": """ - /report <причина>: дайте ответ на сообщение чтобы отправить его админам на проверку.
 - @admin: тоже самое что и /report, только без причины.
ПРИМЕЧАНИЕ: возможность репорта разрешена только пользователям, на админов не будет реакции

*Только для администраторов:*
 - /reports <on/off>: изменить установки репорта, или просмотреть данный статус.
 - Если вывести в личку бота - изменить свои установки.
