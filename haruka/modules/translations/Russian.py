...
"Antispam security_help":"""*Только для администраторов:*
 - /antispam <on/yes/off/no>: Отключить защиту от нежелательного спама, или показать данные установки если нет аргумента. 

Антиспам используется владельцами ботов для бана спамеров во всех группах. Это помогает защитить \
вас и ваши группы, удаляя спам флудеров как можно скорее. Это может быть отключено для вашей группы используя \
`/antispam off`""",

"AntiFlood_help": """- /flood: Получить данные настройки антифлуда.

*Только для администраторов:*
 - /setflood <число/`no`/`off`>: Включить или отключить антифлуд защиту.""",
...
*Только для администраторов:*
 ...
 - /tban <пользователь> (m/h/d): Забанить пользователя на X времени. m = минуты, h = часы, d = дни.
 - /unban <пользователь>: Разбанить пользователя. 
 - /kick <пользователь>: Кикнуть пользователя.
ПРИМЕЧАНИЕ: Вместо конструкции  `/команда <пользователь>` можно ответить командой на любое сообщение пользователя.""",
...
"Command disabling_help":""" - /cmds: Проверка данного состояния отключенных команд.
*Только для администраторов:*
 - /enable <команда>: включить эту команду.
 - /disable <команда>: отключить эту команду.
 - /listcmds: список всех возможных команд для отключения.""", 
...
"Locks_help": """ - /locktypes:  список возможных блокировок.

*Только для администраторов:*
 - /lock <тип>:  Заблокировать элемент в этом чате.
 - /unlock <тип>: Разблокировать элемент в этом чате.
 - /locks: Данный список блокировок в этом чате.

Блокировку можно использовать для ограничения пользователей группы. Например:
Блокировка URL-адресов будет автоматически удалять все сообщения с URL-адресами, которые не были в белом списке, блокировка стикеров будет удалять все \
стикеры и т. п.
Блокировка ботов не остановят администраторов от добавления ботов в чат.""",
...
"Log Channels_help": """*Только для администраторов:*
- /logchannel: Получить информацию о данном лог канале
...
"Misc_help": """ - /id: Получить ID группы. Если ответить командой на сообщение-вы получите ID пользователя.
 - /runs: Ответить рандомной фразой.
 - /slap: Ударить пользователя.
 - /time <место>: Выводит местное время.
 - /info: Получить инфу о пользователе.
 - /gdpr: Удаляет инфу о вас из базы данных бота.
 - /stickerid: Ответьте на стикер чтобы получить его ID.
 - /getsticker: Ответьте на стикер чтобы получить его как png.

 - /markdownhelp: Краткая информация о том, как работает markdown в Telegram, - можно вызвать только в ЛС.""",
...
Если вы хотите получить содержимое заметки без форматирования, используйте '/ get <название заметки > noformat'. Это \
полезно при обновлении существующей заметки.

*Только для администраторов:*
 - /save <название заметки> <данные заметки>: Сохранить заметку.
Кнопка может быть добавлена в заметки с помощью стандартного синтаксиса ссылки на рохмітку - ссылка должна быть просто добавлен с помощью \
`buttonurl:` пример: `[Кнопка](buttonurl:example.com)`. Напишите /markdownhelp Для получения расширенной информации.
 - /save <название заметки>: ответьте на сообщение чтобы сохранить
...
"Purges_help": """*Только для администраторов:*
 - /del: удалить сообщение, на которое вы ответили.
 - /purge: удаляет все сообщения между этим и тем на которое вы ответили
 - /purge <Число Х>: удаляет сообщение на которое вы ответили и следующие х-сообщений. """
